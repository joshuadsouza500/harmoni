...
const FAQ = () => {
  return (
    <Bounded className="h-auto pt-8 md:pt-14 2xl:pt-16  pb-16  xl:pb-20 ">
      <div className="flex flex-col lg:flex-row items-start justify-between  gap-14 lg:mx-2">
        <div className="w-full lg:w-1/2 pt-10">
          <h1 className="text-3xl md:text-4xl font-medium mb-10 lg:mb-12 text-gray-900">
            Frequently Asked Questions
          </h1>
          <div className="w-full max-w-3xl lg:pl-4">
            <Accordion type="single" collapsible>
              {faqData.map((faq, index) => (
                <AccordionItem key={index} value={`item-${index}`}>
                  <AccordionTrigger className="text-lg lg:text-xl font-medium text-gray-900 hover:text-gray-600 lg:pt-4 lg:pb-4">
                    {faq.question}
                  </AccordionTrigger>
                  <AccordionContent className="text-gray-600 pl-0.5 tracking-wider">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
        ...
        <div className="hidden lg:block  w-1/2 2xl:w-2/5 max-2xl:pl-6">
          <div className="relative aspect-square bg-gray-50 rounded-2xl overflow-hidden h-[500px] shadow-md">
            <Image
              height={400}
              width={400}
