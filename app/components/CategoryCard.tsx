...
const CategoryCard = ({ index, name, image, className }: CategoryCardProps) => {
  return (
    <CarouselItem
      key={index}
      className={cn(
        "group relative   rounded-lg transition-all duration-300 hover:shadow-lg basis-1/2 md:basis-1/4",
        className
      )}
    >
      <div
        className="min-w-52 min-h-52 md:min-w-56 md:min-h-56 xl:min-w-64 xl:min-h-64 aspect-square    relative  bg-cover bg-center bg-no-repeat transition-transform duration-300 group-hover:translate-x-0.5 rounded-lg  "
